...
FALLBACK_API_KEY = "AIzaSyDREAL-API-KEY-FROM-GOOGLE"  # ✅ کلید واقعی اینجا وارد شده
...
# فقط placeholderهای واقعی را اینجا نگه‌دار
KNOWN_PLACEHOLDERS = ["YOUR_API_KEY_HERE", "AIzaSyBLcYcWcytb-KAHkGg5e_9tvSoKkogAQ9s_REPLACE_THIS"]
...
def configure_api():
    ...
    if not api_key_to_use:
        ...
        st.warning("⚠️ کلید API از متغیر محیطی `GEMINI_API_KEY` خوانده نشد...")
    
    current_known_placeholders = KNOWN_PLACEHOLDERS + ([FALLBACK_API_KEY] if "AIzaSyBL" in FALLBACK_API_KEY else [])
    if api_key_to_use and api_key_to_use not in current_known_placeholders and len(api_key_to_use) > 20:
        try:
            genai.configure(api_key=api_key_to_use)
            return True
        except Exception as e: st.error(f"❌ خطای پیکربندی API: {e}"); return False
    else:
        st.error("❌ کلید API (GEMINI_API_KEY) تنظیم نشده یا نامعتبر است.")
        st.markdown("به [مستندات Google AI](https://ai.google.dev/tutorials/setup) مراجعه کنید.")
        return False
...
def extract_name_from_resume(resume_text: str) -> str:
    """
    سعی می‌کند از ۵ خط اول رزومه نام فرد را استخراج کند.
    اگر موفق نشود، مقدار پیش‌فرض باز می‌گرداند.
    """
...
"""
    ...
    try:
        ...
        response = model.generate_content(prompt_text, **DEFAULT_REQUEST_OPTIONS)
...
def handle_id_login_input():
    ...
    if st.button("جستجو و شروع مصاحبه با شناسه", key="start_with_id_button"):
        if not user_id_input: st.warning("لطفاً شناسه کاربری خود را وارد کنید."); return
        st.session_state.user_id = user_id_input
        try:
            with open("data.json", "r", encoding="utf-8") as f: data = json.load(f)
            user_data = data.get(st.session_state.user_id)
            if user_data and user_data.get("resume"):
                ...
                else: st.session_state.selected_job = None; st.session_state.selected_job_id = None
                if not st.session_state.selected_job_id:
                    st.warning("موقعیت شغلی برای این شناسه در data.json یافت نشد یا نامعتبر است.")
                    job_options = list(JOB_PROFILES.keys()); selected_job_key = st.selectbox("لطفاً موقعیت شغلی را انتخاب کنید:", job_options, key="job_selection_id_path", index=None, placeholder="انتخاب کنید...")
                    if selected_job_key: st.session_state.selected_job = selected_job_key; st.session_state.selected_job_id = JOB_PROFILES[selected_job_key]
                    else: st.error("برای ادامه باید یک موقعیت شغلی انتخاب شود."); return
                st.session_state.interview_stage = "initial_greeting"; st.rerun()
            else: st.error(f"❌ شناسه '{st.session_state.user_id}' یافت نشد یا رزومه ناقص است.")
        except FileNotFoundError: st.warning("⚠️ فایل data.json یافت نشد.")
        except json.JSONDecodeError: st.error("❌ خطا: فایل data.json فرمت نامعتبر دارد.")
        except Exception as e: st.error(f"⚠️ خطا در پردازش اطلاعات کاربر: {e}")
...
# فقط اگر منتظر پاسخ هستیم، ورودی از کاربر بگیر
    if st.session_state.awaiting_screening_answer:
        user_input = st.chat_input("پاسخ شما یا سوالتان...")
        
        if user_input:
            st.session_state.screening_conversation.append({"sender": "user", "message": user_input})

            if any(w in user_input for w in ["?", "؟", "یعنی", "توضیح", "چی", "چطوری"]):
                st.session_state.screening_conversation.append({"sender": "bot", "message": "حتماً! اگر نیاز به توضیح بیشتری دارید، خوشحال می‌شم کمک کنم. لطفاً دقیق‌تر بپرسید."})
                st.rerun()
                return

            bad_answers = ["نه", "خیر", "ندارم", "نمی‌دونم", "نمیدونم", "اوکی", "ok", "باشه", "فعلا نه"]
            if len(user_input.strip()) < 6 or user_input.strip().lower() in bad_answers:
                st.session_state.screening_conversation.append({
                    "sender": "bot",
                    "message": "ممنونم، ولی لطفاً پاسخ را کامل‌تر و دقیق‌تر بنویسید تا بتوانیم بررسی کنیم."
                })
                st.rerun()
                return
    ...
    # اگر پاسخ خوب بود، ذخیره و سوال بعدی
    q = st.session_state.current_screening_question
    st.session_state.screening_answers.append({
        "question_id": q["id"],
        "question_text": q["text"],
...
def display_interview_interface():
    ...
    if current_stage == "initial_greeting":
        ...
        کمی در مورد خودم بگویم — من یک مصاحبه‌گر آزمایشی هوش مصنوعی هستم.  
...
"""
    try:
        with st.spinner("در حال انجام تحلیل نهایی توسط هوش مصنوعی..."):
            model_analyzer = genai.GenerativeModel("gemini-1.5-pro-latest")
            response_analysis = model_analyzer.generate_content(analysis_prompt_template, request_options=DEFAULT_REQUEST_OPTIONS)
...
def main():
    ...
    if not configure_api():
        st.title("🤖 مصاحبه‌گر هوشمند شرکت داده پردازان بنیان آوا"); st.markdown("---"); st.error("پیکربندی API ناموفق بود."); return
    ...
    if current_stage == "initial_setup":
        ...
        st.title(f" مصاحبه با {st.session_state.user_name}{job_title_display}")
        st.markdown("---"); 
        display_interview_interface() 
    else: 
        st.error("وضعیت نامشخص برنامه."); 
        offer_reset_confirmation()
